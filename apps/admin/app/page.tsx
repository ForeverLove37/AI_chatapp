...
type SearchProvider = {
  ...
  slug: string;
  displayName: string;
  kind: "duckduckgo" | "tavily" | "serpapi";
  endpoint: string;
  priority: number;
  maxResults: number;
  enabled: boolean;
...
const englishCopy: AdminCopy = {
  ...
  recoveryDetail: "Disaster recovery procedure for Adaptive Chat backup format v1", stopWriters: "Stop stateful writers", stopWritersDetail: "Stop the application services with the command below. Keep PostgreSQL and Redis available.", decryptAuthenticate: "Decrypt and authenticate", decryptDetail: "Run the command below and enter the original destination passphrase when prompted. Authentication failure aborts without producing a dump.", restorePostgres: "Restore PostgreSQL", restorePostgresDetail: "Restore the authenticated dump with the command below.", restartValidate: "Restart and validate", restartValidateDetail: "Restart the application services, then verify health, sign-in, dynamic channels, and a test background job.", recoveryNote: "Redis queue data is operational state. PostgreSQL background job records are authoritative, and queued or running jobs are recovered automatically when the worker starts.",
...
const chineseCopy: AdminCopy = {
  ...
  recoveryDetail: "Adaptive Chat 备份格式 v1 的灾难恢复流程", stopWriters: "停止有状态写入服务", stopWritersDetail: "使用下方命令停止应用服务，并保持 PostgreSQL 和 Redis 可用。", decryptAuthenticate: "解密并验证", decryptDetail: "运行下方命令，并在提示时输入备份目标的原始口令。验证失败会立即终止且不会生成转储文件。", restorePostgres: "恢复 PostgreSQL", restorePostgresDetail: "使用下方命令恢复已验证的转储文件。", restartValidate: "重启并验证", restartValidateDetail: "重启应用服务，然后验证健康检查、登录、动态频道及一个后台测试任务。", recoveryNote: "Redis 队列数据属于运行状态。PostgreSQL 中的后台任务记录是权威数据，Worker 启动时会自动恢复排队中或运行中的任务。",
  ...
  return <div className="enterprise-stack"><InlineError message={error} />
    <section className="data-section"><SectionHeading title={editingId ? copy.editSearchProvider : copy.addSearchProvider} detail={copy.searchPriorityDetail} />
      <form className="form-grid search-provider-form" onSubmit={submit}>
        ...
        <label>{copy.providerKind}<select value={form.kind} onChange={(event) => selectKind(event.target.value as SearchProvider["kind"])}><option value="duckduckgo">DuckDuckGo</option><option value="tavily">Tavily</option><option value="serpapi">SerpApi</option></select></label>
